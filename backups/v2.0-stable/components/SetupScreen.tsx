...
export const SetupScreen: React.FC = () => {
      ...
      {[1, 2, 3, 4].map((s) => (
        <div 
          ...
          className={`h-2 rounded-full transition-all duration-500 ${
            ...
            s < step ? 'w-2 bg-primary-200' : 'w-2 bg-slate-200 dark:bg-slate-700'
          }`}
        />
      ))}
    </div>
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-4 bg-slate-50 dark:bg-slate-900 transition-colors duration-300">
      <div className="w-full max-w-3xl bg-white dark:bg-slate-800 rounded-2xl shadow-xl p-6 sm:p-10 border border-slate-200 dark:border-slate-700 transition-all duration-300">
        ...
        <div className="text-center mb-8">
          <h1 className="text-2xl font-bold text-slate-800 dark:text-white tracking-tight mb-2">
            ...
            {step === 3 && "Personalidad del Guardián"}
            {step === 4 && "Preferencia Visual"}
          </h1>
          <p className="text-slate-500 dark:text-slate-400 text-sm">
            {step === 1 && "Define tu moneda y entorno base."}
            {step === 2 && "Pinta los días que NO trabajarás este año."}
            {step === 3 && "¿Cómo quieres que la IA te hable?"}
            {step === 4 && "Elige el modo que mejor se adapte a tu vista."}
          </p>
        </div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  {(['UYU', 'USD', 'UI', 'UR'] as Currency[]).map((curr) => (
                    <button
                      ...
                      className={`p-4 rounded-xl border text-left flex justify-between items-center transition-all duration-200 ${
                        localSettings.baseCurrency === curr
              ...
              {[
                ...
                return (
                    <button
                      ...
                      className={`w-full p-5 rounded-xl border-2 text-left flex items-start gap-4 transition-all duration-200 group ${
                        isSelected
        ...
        <div className="mt-10 pt-6 border-t border-slate-100 dark:border-slate-700 flex justify-between items-center">
          ...
          {step < 4 ? (
            ...
            <Button onClick={finalizeSetup} className="px-8 bg-primary-600 hover:bg-primary-700 text-white shadow-lg shadow-primary-600/30">
              Guardar y Continuar
            </Button>
          )}
        </div>
